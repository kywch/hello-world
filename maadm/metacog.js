...
var confidence_response_area =
    '<div class = cntexpbox><div class = fixation>+</div></div><div class = response_div>' +
    ...
    '<button class = response_button id = Confidence_3><b>(3)</b><br>Very<br>confident</button>'
...
var confidence_response_area_key =
    ...
    '<button class = response_button_key id = Confidence_3><b>(3)</b><br>Very<br>confident</button>'
...
var feedback_instruct_text =
    'This task will take about 10 minutes. Press <strong>enter</strong> to begin.'
...
var instructions_block = {
    ...
    pages: [
        ...
        'whether the left or right stimulus has the grating by using <font color=red>the arrow keys</font></b>.</p>' +
        ...
        'how confident you are that you were correct, on a scale from 1 (not confident) to 3 (very confident). ' +
        '<b>Press <font color=red>the corresponding number key</font> to indicate your confidence.</b></p> ' +
...
function practice_block(answer) {
    return {
        ...
        prompt: '<div class = cntexpbox><div class = fixation>+</div>' +
            '<p class = center-block-text>Please use the left and right arrow keys to indicate the side with stripes.</p></div>',
        timing_post_trial: 0
    };
}
...
var confidence_key_block = {
    ...
    response_ends_trial: true,
    timing_post_trial: 0,
    prompt: '<div class = cntexpbox><p class = center-block-text>Please use the number keys to indicate your confidence.</p></div>',
    on_finish: function(data) {
        var index = confidence_choices.indexOf(data.key_press)
        jsPsych.data.addDataToLastTrial({
            confidence: 'confidence_' + (index + 1)
        })
    }
}
