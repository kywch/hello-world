...
for (var ii = 0; ii < option_practice.length; ii++) {
    var choice_page = {
        ...
        choices: function() {
            ...
            return getChoiceKeys(option_practice[curr_prac_choice]);
        },
        stimulus: function() {
...
/*
===================================
THE MODIFIED TASK, testing for fMRI
===================================
*/

// types of trials: cue or decision, math or word, easy or hard, 2 or 5
var nochoice_type = [];
var nochoice_time = []; // decision time
...
var nochoice_instruct_page = {
    ...
    text: "<div class = centerbox>" +
        ...
        "where the option is presented using the left or right key within 3 seconds. " + 
        "You will earn the points if you get the problem correct.</p>" +
...
*/
//var pcate_options = ['Math', 'Word'];
var nochc_left_type = ['Easy', 'Easy', 'Hard', '', '', '', 'Easy', 'Easy', 'Hard', 'Hard'];
var nochc_left_stake = [2, 5, 5, -1, -1, -1, 2, 2, 5, 5];
var nochc_right_type = ['', '', '', 'Easy', 'Easy', 'Hard', 'Hard', 'Hard', 'Easy', 'Easy'];
var nochc_right_stake = [-1, -1, -1, 2, 5, 5, 5, 5, 2, 2];
var curr_nochc = '';

var option_nochc_seed = [];
for (var iCate = 0; iCate < pcate_options.length; iCate++) {
    for (var iStk = 0; iStk < nochc_left_stake.length; iStk++) {
        option_nochc_seed.push({
            probCate: pcate_options[iCate],
            typeLeft: nochc_left_type[iStk],
            stakeLeft: nochc_left_stake[iStk],
            typeRight: nochc_right_type[iStk],
            stakeRight: nochc_right_stake[iStk]
        });
    }
}

for (var iB = 0; iB < (num_block - num_cast); iB++) {
    option_main.push(shuffle(option_nochc_seed));
}


/* Modified task, which include the no-choice trials */
function generate_nochoice_block(block_count, flag_feedback) {
    // assume that option_main is defined
    var opt_list_name = "option_main[" + block_count.toString() + "]";
    var option_this = eval(opt_list_name);
    var block_sequence = [];
    ...
    var start_main_block_page = {
        ...
        text: function() {
            ...
            return "<div class = centerbox><p class = block-text>" +
                ...
                "<p class = block-text>In this block, you will be presented with <b>" + option_this.length.toString() +
                "</b> trials. " +
                "<p class = block-text>Some of them are the <i>no-choice</i> trials, in which the problems are pre-determined." +
                "In those trials, you should indicate where the option was presented by pressing the left or right key " +
                "<b>within 3 seconds</b>, otherwise you will solve the Easy problem with 1 point at stake.</p>" +
                block_fdbk_str +
                "<p class = block-text>The maximum points you can earn in this block is <b>88</b> points. </p>" +
                "<p class = block-text>Press &quot;<b>o</b>&quot; key to begin a trial.</p>" +
                "</div>";
        },
        timing_post_trial: 1000,
        data: {
            exp_stage: 'nochoice_block_' + (block_count + 1).toString() + '_start'
        },
        on_finish: function() {
            console.log("Starting the Main block... ");
            if (flag_debug) {
    ...
    for (var ii = 0; ii < option_this.length; ii++) {
        var choice_page = {
            ...
            choices: function() {
                // should be ['i', 'p'] for the normal options
                return getChoiceKeys(eval(opt_list_name + "[curr_main_hard]"));
            },
            stimulus: function() {
                option_current = eval(opt_list_name + "[curr_main_hard]");
                if ((option_current.stakeLeft * option_current.stakeRight) > 0) {
                    curr_nochc = 'c'; // choice (choose-and-solve trial)
                } else {
                    curr_nochc = 'a'; // no-choice (anticipation trial)
                }
                return "<div class = centerbox><p class = block-text>Choose your option. " +
                    "Press 'i' for the left and 'p' for the right option within 3 seconds.</p>" +
                    generateChoiceHTML(option_current) + "</div>";
            ...
            on_finish: function(data) {
                ...
                let tmp_type = curr_nochc + curr_probcate[0] + curr_choice[0] + curr_at_stake.toString();
                nochoice_type.push(tmp_type);
                nochoice_time.push(data.rt);
                if (flag_debug) {
                    console.log("Current trial type: ", tmp_type);
                }
            }
        }
        block_sequence.push(choice_page);
    ...
    var finish_main_block_page = {
        ...
        text: function() {
            ...
            return "<div class = centerbox><p class = block-text>" +
                ...
                correct_block.toString() + "</font></span> out of 20 problems correct</p>" +
                "<p class = block-text>and earned <span class='very-large'><font color='blue'>" +
                point_block.toString() + "</font></span> out of 88 points. </p>" +
                "<p class = block-text>So far, you earned <span class='very-large'><font color='blue'>" +
                point_main.toString() + "</font></span> points. </p>" +
                "<p class = block-text>Press <strong>Enter</strong> or <strong>Space bar</strong> key to " +
                block_finish_str + "</div>";
        ...
        data: {
            exp_stage: 'nochoice_block_' + (block_count + 1).toString() + '_finish'
        },
        timing_post_trial: 1000,
        on_finish: function() {
            save_data();
